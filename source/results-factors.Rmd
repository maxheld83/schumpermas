...
2. Centroid Factor Analysis (CFA)
...
I'm a little behind the curve on the listserv, but I, too, am still struggling with the supposed "indeterminacy" of centroid (CFA) vs. principal components (PCA). I don't have anything to add to Noori's succinct question or the 2012 Brown/Schmolck CFA vs PCA exchange (kindly posted by Noori, highly recommended).

I've spend a couple of hours sifting through the listserv and am trying to offer an update on the state of the centroid vs PCA/PAF debate.

Is the following summary correct/fair?
The supposed indeterminacy of the  extraction method is the primary reason for proponents of CFA. (In addition, CFA but not principal axis factoring (PAF), works with commonalities in the diagonal and more people than Q-sorts.)
The supposed indeterminacy does not lie in a rotation phase specific to PAF; PCA, too, admits of an infinite number of rotations, from which one must be chosen for mathematical (varimax?) and/or theoretical (judgmental) reasons. (Peter Schmolck helpfully unearthed a source way back from 1938 to that effect in the above 2012 exchange; McCloy, Metheny & Knott 1938).
CFA comes in different variants, depending on what is placed on the diagonal of the correlation matrix (communalities, test-retest reliabilities, ... -> see above 2012 thread by Schmolck on more options, as well as Brown 1980: 211). It would appear that the appropriate choice of diagonal values comes down to an ontological question on whether there are, or are not, latent variables in Q (see recent, also unresolved, thread by Dean and Schmolck), and would be analogous to the choice between PCA and PAF.
CFA includes manual "reflections" of variables in the correlation matrix, by which the signs of columns and rows are reversed so as to have as many possible rows sum to a positive number (see Brown 1980: 209 for details). The procedure effects that some people's Q-sorts are reversed, or reflected around the center: the degree (absolute magnitude) of association between people remains, but the sign changes. Reflecting, obviously, may cause other rows to have negative sum, but the sum total may be increased. After the first factor is extracted, the residual correlation table (as well as the appropriate factor loadings) are de-reflected. The procedure is repeated for every extracted factor.
It seems to me that only point 4 is uniquely "indeterminate" to CFA; variates can be reflected in a different order, with more or less skill, yielding different (unrotated) factor loadings. (Notice, though, that as Noori and Peter intimated, there ought to be one set of reflections that yields the highest possible variance, even if CFA has no way of producing that).

For this feature of CFA to be valuable to researchers, I suggest two conditions:
a) the procedure would have to be practicable.
b) the procedure would have to be intelligible to researchers in substantive terms to aid them in their abduction.
Re: a): It seems to me that few researchers today would be inclined to do even a CFA by hand, including the manual reflections. We would need a computer program that allows manual reflection, which would be possible, though the user interface may be non-trivial and I am not aware that such a program exists.

Re: b): It is utterly inconceivable to me how a researcher should make abductively informed reflections this early in the analysis; the table acted upon (the correlation matrix) and the operation (multiplying of rows and columns by -1) does not, to me, reveal and/or correspond to the kind of substantive interpretation of factor scores in which every Q study culminates, and in which, supposedly, the abduction occurs (as in: "mmh, interesting, under this viewpoint item 02 and item 04 are pretty close, I wonder what might make people think so ..."). I've spent the last 9 month with my modest keyneson dataset (77 statements, 19 people), and I'm still having a hard time relating this or that methodological decision (say, flagging or judgmental rotation) to substantive outcomes in terms of factor scores. I have no idea what kind of abductive insight I could possibly gain during manual reflection.
There is, of course a (sinister) alternative: you could go back and forth between reflections and downstream factor scores and pick whichever reflection results in factors that make sense. That would, first of all, be colossally cumbersome. Also, I fear, that might stretch the concept of abduction pretty thin – it's more like data mining.

Note, again, that judgmental rotation, what ought to be in the diagonal (aka PAF vs PCA) as well as an abductive process in general are entirely separate concerns. The question is what centroid brings to the table in addition or as a condition for the former.

It seems to me: nothing much but confusion.

Steven Brown wrote on this list in 1997 in favour of centroid that "there may be more to mathematics than mathematics". That may be so, but I would think that any such epistemological or ontological differences should somehow play out in research practice in a meaningful way. Manual reflection is the only difference between PCA/PAF and CFA I can see in how a researcher proceeds, and I imagine it to be quite meaningless as described above.

It's entirely possible that I'm completely wrong. In that case, I apologize for my ignorance in advance and blame it partly on the relative paucity of clear treatments on the matter.

Attentive readers of the listserv will notice that much of this is a reiteration of Peter Schmolck's thoughts over the years (including from 1997); I (and, I think, Q) are deeply indebted to him for his thoughtful dissenting opinions. Insight is his, mistakes are mine.

Best
Max

Ps.: Bonus points for responses that avoid hand-waving references to "R" vs "Q".

McCloy, Metheny & Knott 1938; / Brown;
> The fact of indeterminacy in its free-floating usage by Stephenson is commonplace, at least nowadays.  Incidentally I came across an article that made clear already in the year 1938 that "the factors found by the Hotelling method of principal components present the same necessity for rotation as those found by the Thurstone method of multiple factors. (McCloy,  Metheny,  & Knott, 1938).
>


Brown 1980: 209
> The centroid method was the first to be fully elaborated in factor theory, largely because of its computational ease, although it was always regarded as only an approximation to the more refined methods such as principal axes (Holzinger, 1946); on the other hand, Burt (1972) has more recently shown that the centroid and the more precise methods produce virtually the same results. With the development of computers, however, what was before a chore came within easy reach of all, so that contemporary factorists can now avail themselves of the more determinant methods, and usually do.


Schmlock in 1997 on the list:
> Quite to the contrary: I would
like to hear what the master's (Q methodology) orders for the slave
(mathematics) are. I do not object against the demand that the data
should _not_ (automatically) determine results. However, that does not
suffice as a goal definition for which a mathematical solution exists.
The point I wanted to make is really very trivial: One cannot use
mathematics to solve a non-mathematical problem without specifying one's
goals and criteria in mathematical terms. Everything else would amount
to number-magic. Steven Brown's eloquent exposition of the scientific
goals of interbehaviorism is beside the point here. His explanations
might be of relevance only for someone who believes mathematics is the
master, and that mathematical criteria can decide about the legitimacy
of scientific goals. I am not aware of anyone who thinks so.
- https://listserv.kent.edu/cgi-bin/wa.exe?A2=Q-METHOD;624d5964.9706
...
> "... but in Q methodology it is often worthwhile to rotate factors judgmentally in keeping with theoretical, as opposed to mathematical, criteria [emphasis added] (Creaser, 1978; Thompson, 1962). In these respects, the centroid method, long ago discarded for its indeterminacy, still has its usefulness, for the uniqueness of the centroid method is its indeterminacy [emphasis added]: There is no correct solution out of the infinite number of solutions available, so the investigator is free to pursue his own inclinations, guided by his theory.
> - Brown 1980 p. 33
> -

Notice that is pretty unclear just how manual reflection should be theoretically informed -- it's just too far away from that data.

In any event, no one is using manual CFA – they all use PQMethod, which implements an automatic reflection procedure.

Underlying simple structure is Occam's razor.

CFA was originally introduced as a "computational expedient when it became apparent that the principal-factor solution was too laborious" (Harman 1967: 5) -->
...
<!-- Peter Schmolck describes (the justification of) centroid as a "myth" and "dead freight" (email from June 2nd, 2012) -->
<!-- more on this: https://listserv.kent.edu/cgi-bin/wa.exe?A2=ind1206&L=q-method&P=R2&1=q-method&9=A&J=on&d=No+Match%3BMatch%3BMatches&z=4 -->

<!-- Noori Akthar-Danish is also very skeptical on centroid, not clear what exactly the supposed *indeterminance* would mean -->
...
<!-- The manual steps in centroid are when manually "reflecting" columns – and that is a step that involves some amount of discretion, can be done more or less skillfully.
As a result, factors also need not decrease monotically.

The argument for manual reflection kind of goes away, because nowadays centroid is always done with a computer anyway.

Peter Schmolck describes the "centroid myth" as dead freight. (email from june 2nd, 2012) https://listserv.kent.edu/cgi-bin/wa.exe?A2=ind1206&L=q-method&F=&S=&X=7A14F22EEE7751DED2&Y=p41bsmk%40unibw-muenchen.de&P=63 -->

<!-- MH TODO find real resources on PCA vs centroid -->
