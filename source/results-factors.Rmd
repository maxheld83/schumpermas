...
It is incorrect, what brown 1980: 211 writes
> As indicated previously, the correlation of each variable with itself would produce all 1.00s in the diagonal, and some factor analysts prefer this "closed model" (Cattell, 1965: 198), which assumes no error.
> As a practical matter, however ,it is doubtful that any person in real life, if usntructed to take the same Q sort wtice (which adminsitrations separated by a day or two), would ever correlate with himself as highly as 1.00; we would expect test-retest *r*s (relibaility coefficients) to be more nearly in the neighborhood of 0.80 to 0.90 (Frank, 1956; Hilden, 1958; Steller and Meurer, 1974), and if these *r*s where available for each subject, they would be the preferred diagonal elements.
...
Much of the debate, unfortunately happens only on the email list, especially through Schmolck.
