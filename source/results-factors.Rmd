...
if (real_names == TRUE) {
  ...
  # TODO make this as a reader, not replace it
  # TODO also do this for feedback
}
```
...
<!-- TODO make a correlation matrix, just for descriptives, look at which items are most highly correlated same thing for q sorts -->
...
The following pages will hopefully assuade the Q skeptics, and take along newcomers to the method.
...
To make sense --- as we must --- of the shared viewpoints on taxation and the economy among the participants of the first CiviCon Citizen Conference, we must first be sure what they are, and that they are, in fact, shared.
...
## Correlation
 ...
 correlation matrix for factor analysis, Q is still an exotic research method.

1. *Transposed Matrix*. While methods of exploratory factor analysis (EFA) are somewhat advanced, but well-established in mainstream quantitative social research, Q
2.
Q method requires a more careful, patient treatment, because its statistics are different on a number of counts:



Both because Q method is little known, and to live up to the promise of a *constrained*, qualitative analyses, I will track each statistical procedure in the following quite narrowly.

Literate statistics.

This will hopefully assuade the doubts of skeptics, and take along newcomers to the methods.
At the same time, some of the details of this chapter may raise the ire of some established Q methodologists who never tire of stressing the substantive analysis over statistical flourishes.
They are right: the key to understanding human subjectivity lies in abductive intelligence, in thoroughly going back and forth between informed hunches and what the data will bear out.
Still, statistical rigor may be a worthwile *necessary*, though not *sufficient* condition for a good Q method.
...
<!-- mention controversy about Q, latest piece -->
...
THis will be more verbose than usual.
...
<!-- Why would we get rid of negative cors? What do the columns mean? Brown 210 odd prcedure -->
