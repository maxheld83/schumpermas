...
Factor analyses or principal components analyses are well-established procedures and will not usually receive a lot of explanation in a results section.
In Q – almost 100 years after its foundation, still an *exotic* procedure – things are a little different.
Because Q method transposes the correlation matrix, positing *people* as *variables*, and *items* as *cases*, all of the downstream concepts in data reduction, from covariance to eigenvalues, take on a different, Q-specific meaning, even if the mathematics stay the same.
...
There will be some math, but limited and a lot of "impressionistic" talk, and analogies.
This seems bad style, unscientific, just math would be better.
It seems hard to conceive of a way to track all the steps from "deliberative subjectivites" to R2 without some appeal to intuition and analogy on the way, at least partly because human subjectivity on economic policy is not fully mathematized, not in this dissertation, hardly (or unconvincingly) in the broader field, and maybe never.
