...
cor.ex$identity <- c(rep(x = vec.examples$minimum[2], times = 2))  # identity is just one with him/herself
...
Principal components analysis can be (equivalently) described in at least two formalisms: linear algebra and matrix algebra, both of which can be illustrated with the present Q data.


#### Linear Algebra

```{r SE-example, include=FALSE, eval=FALSE}
# # http://stats.stackexchange.com/q/153564/60119
# q_sorts[,c("Elena", "BarbaraK"), "before"]
# Person1 <- q_sorts[, "BarbaraK", "before"]
# names(Person1) <- NULL
# Person2 <- q_sorts[, "Elena", "before"]
# names(Person2) <- NULL
# Person1 <-paste(Person1, collapse = ",")
# Person2 <- paste(Person2, collapse = ",")
# Person1
# Person2
####
# I have a dataset of 17 people, ranking 77 statements. 
# I want to extract principal components on a *rotated* correlation matrix of correlations *between people* (as variables) *across statements* (as cases).
# I know, it's odd, it's called [Q Methodology](http://en.wikipedia.org/wiki/Q_methodology).
# 
# I want to **illustrate** how PCA works in this context, by **extracting and visualizing eigenvalues/vectors** for only a *pair* of data.
# (Because few people in my discipline *get* PCA, let alone it's application to Q, myself included).
# 
# I want the visualization from [this fantastic tutorial](https://georgemdallas.wordpress.com/2013/10/30/principal-component-analysis-4-dummies-eigenvectors-eigenvalues-and-dimension-reduction/), only for **my real data**.
# 
# ![Great visualization][1]
# 
# Let this be a subset of my data:
#
    ...
    g
# 
# ![corrplot for my data][2]
# 
# Notice that, by measurement, this data:
# 
# - ... has a mean of zero, 
# - ... is perfectly symmetrical, 
# - ... and is equally scaled on both variables (should be no difference between correlation and covariance matrix)
# 
...
# ![final plot][3]
# 
# Obviously, something is wrong with this plot:
# 
# - the vectors don't seem to slice either of the ellipses in half
# - the **vectors do not appear to be orthogonal!**
# 
# So, what did I do wrong?
# What could I do better?
# Is this a reasonable visualization?
```

```{r eigen-illustration}
source("qmethod/R/q.eigenpair.R")  # until properly in function
vec.examples <- q.eigenpair(data = q_sorts[,,"before"])
do.call("grid.arrange", c(vec.examples$scatterplots, ncol=2))
```
