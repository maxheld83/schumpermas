...
#   slabels <- row.names(data)  # make labels vector
#   slabels[-sample(x = 1:length(row.names(data)), size = nlabels, replace = FALSE )] <- ""  # NA#s would be cleaner but shout later on
  ...
  for (i in pairs) {
    ...
    g <- NULL
    g <- ggplot(data = as.data.frame(data), mapping = aes_string(x = i[1], y = i[2], label = deparse(rownames(data))))
    ...
    g <- g + geom_text()  # add sampled labels
