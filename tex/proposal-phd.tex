...
%deliberation is a last end; it is a normative ideal against which reality is compared.
%it is not in any straightforward way an iv or a dv.
%you don't add (information) or substract something (moderators) from ordinary political debate (whatever that would be), and see how it affects, say, conflicts. instead you make it a whole different game.
%deliberation is *the standard* by which power can be legitimized.
%it isn't an addition to a different politics; it IS the politics.

%talk about the difference between "practice" and "theory" -- it's overblown; practice is just theory with more incremental changes and/or more complexity etc. In Deliberation, there's no "in theory this", in "practice that".
	%my hunch is that you cannot abstract away substantive deliberative quality into some measure; you need to know the substance and take a position on that substance. 
	%note that I may not be against stories or emotions. consider poletta and lee as support that they may be instrumentally helpful and/or demasio they MAY be the stuff of reasons
	%In a way, the standard is procedural -- but also not. The questions is whether all the arguments and claims that have been raised were answered, in their own terms. That's all.
	%somehow talk about the emancipative promise.
	%review the book on amazon
		%- poorly written
		%- unconvincing, formulaic data
		%- poorly structured literature review, not much new
		%- ultimately, uninformative relationship between normative and empirical. It's muddled.

%consider starting out with a question

%It's a common idea to try to abstract away substantive stuff into process; but that does not work for deliberative democracy -- not if you accept the expansive standard, at least.
%There is two routes to go about this:
	%you make it entirely procedural, as in the BC piece -- which is not satisfactory
	%you make it trivially empirical, as in the other piece.
	%as much as they don't like it, social scientists need to get into the substantive issues, the FIRST order question
	%that means you really need to get into the science business behind it; you need to fairly show the controversies that remain, and open this up to the most dramatic (epistmological, ontological, etc.) criticism.
