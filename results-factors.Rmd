...
Beginning with the most highly correlated pair of Q sorts, `r eigen.examples[["positive","pairs"]]`, the left-hand eigenplot in the below figure displays a cloud of points, scattered around an upward-sloping axis, as would be expected for a positive correlation.

```{r linear-algebra-positive, fig.cap = c("Eigenvector Extraction from Positively Correlated Pair"), echo = FALSE}
...
The relationship between the two sorters `r eigen.examples[["positive","pairs"]]` is here presented as a familiar correlation matrix, with a diagonal of `1`.
...
(@matrix-positive) $$
```{r matrix-positive, echo = FALSE, results = "asis"}
...
Because the principal components are just linear combinations of the original axes, these people-variables, `r eigen.examples[["positive","pairs"]]`, can also be added to the left-side biplot, by placing them according to their respective (unrotated) loadings on the two components (the eigenspace).
...
```{r linear-algebra-lowest, fig.cap = c("Eigenvector Extraction from Least Correlated Pair", echo = FALSE)}
...
(@matrix-negative) $$
```{r matrix-negative, echo = FALSE, results = "asis"}
...
By contrast, consider the below, spurious eigenvalue decomposition of `r eigen.examples[["identity","pairs"]]` with herself in linear and matrix algebra notation.
...
(@matrix-identity) $$
```{r matrix-identity, echo = FALSE, results = "asis"}
...
Circularly, because these two Q sorts are identical, they can be created from *one* synthetic set of displacement instructions, which we might call `r eigen.examples[["identity","pairs"]]`.
...
In this boundary case, the scores for the first Principal Component, are, in fact, not synthetic at all: they are simply `r eigen.examples[["identity","pairs"]]`'s sorting vectors.
...
```{r how many factors?, include = FALSE}
howmany <- NULL

# Separately
howmany$before <- q.nfactors(dataset = q_sorts[,,"before"], cutoff = 8, siglevel = 0.05, cor.method = "spearman")
howmany$after <- q.nfactors(dataset = q_sorts[,,"after"], cutoff = 8, siglevel = 0.05, cor.method = "spearman")
howmany$before$residuals.plots[[1]]

homany$before <- q.nfactors(dataset = q_sorts, cutoff = 8, siglevel = 0.05, cor.method = "spearman")

# Together
bandaf <- cbind(q_sorts[,,"before"], q_sorts[,,"after"])  # take before and after together, this is just for kicks
colnames(bandaf)[1:17] <- paste(colnames(q_sorts), "before", sep = "-")
colnames(bandaf)[18:34] <- paste(colnames(q_sorts), "after", sep = "-")
...
ggplot(data = resparan.long, mapping = aes(x = nvars, y = randomeigen, group = ncomps, colour = ncomps)) + geom_line() + geom_hline(yintercept = 1, colour = "red", ) + geom_vline(xintercept = 77, colour = "green") + ggtitle("N = 77 item-cases")
...
Because there are as many eigenvalues as there are people-variables in a Q study, and because eigenvalues *sum* to that number of people-variables, they can be interpreted in terms of *constituent Q-sorts*: the highest eigenvalue accounts for as much as `r max(howmany$before$eigenvalues$Unadjusted)` Q sorts.
...
The first and highest principal component accounts for a dominant `r max(howmany$before$eigenvalues$R2)` share of the variance, followed by distant second and third principal component.

```{r ev-table-before}
knitr::kable(howmany$before$eigenvalues)
```

By the Kaiser-Gutmann rule, also plotted as a red line in the below figure, we may retain up to `r howmany$before$summary["KaiserGuttmann", "NFactors"]` factors.
...
Entirely compatible with this caveat, eigenvalue-based criteria and parallel analysis do, however, constitute *sufficient* --- nut merely *necessary* --- conditions to identify shared subjectivities.
