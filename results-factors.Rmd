...
civicon_2014 <- NULL
civicon_2014$sorts <- q_sorts
civicon_2014$set <- q_set
library(devtools)
...
```{r linear-algebra-positive, fig.cap = c("Eigenvector Extraction from Positively Correlated Pair"), echo = FALSE, purl=FALSE}
...
row.names(q_set)
make.cards(q.set = q_set[sort(row.names(q_set)), ], study.language = "english", output.pdf = TRUE, show.handles = TRUE)
